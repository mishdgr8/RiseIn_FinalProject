...
use crate::generated::errors::RiseinCertificationError;
use crate::generated::instructions::RiseinCertificationInstruction;
...
impl Processor {
    pub fn process(
        ...
        let instruction = RiseinCertificationInstruction::unpack(data)?;

        match instruction {
			RiseinCertificationInstruction::MintCertification(args) => {
				msg!("Instruction: MintCertification");
				Self::process_mint_certification(
					program_id,
					accounts, 
					args.project_name,
					args.my_name,
			...
			RiseinCertificationInstruction::ClaimCertification => {
				msg!("Instruction: ClaimCertification");
				Self::process_claim_certification(program_id, accounts)
			}
			RiseinCertificationInstruction::TransferCertification => {
				msg!("Instruction: TransferCertification");
				Self::process_transfer_certification(program_id, accounts)
			}
			RiseinCertificationInstruction::BurnCertification => {
				msg!("Instruction: BurnCertification");
				Self::process_burn_certification(program_id, accounts)
			}
        }
    }
...
/// - course_description: [String] 
	pub fn process_mint_certification(
		program_id: &Pubkey,
		accounts: &[AccountInfo],
		project_name: String,
		my_name: String,
		...
		if fee_payer_info.is_signer != true {
			return Err(RiseinCertificationError::InvalidSignerPermission.into());
		}
		...
		if *certificate_info.key != certificate_pubkey {
			return Err(RiseinCertificationError::NotExpectedAddress.into());
		}
		...
		if *fee_payer_info.owner != Pubkey::from_str("11111111111111111111111111111111").unwrap() {
			return Err(RiseinCertificationError::WrongAccountOwner.into());
		}
		...
		if mint_info.data_len() != spl_token::state::Mint::LEN {
			return Err(RiseinCertificationError::InvalidAccountLen.into());
		}
		...
		// Calling STUB
		mint_certification::mint_certification(
			program_id,
			&vec![
				mint_info,
			],
...
/// 13. `[]` csl_spl_token_v_0_0_0: [AccountInfo] Auto-generated, CslSplTokenProgram v0.0.0
	pub fn process_claim_certification(
		program_id: &Pubkey,
		accounts: &[AccountInfo],
	) -> ProgramResult {
		let account_info_iter = &mut accounts.iter();
...
/// 12. `[]` csl_spl_token_v_0_0_0: [AccountInfo] Auto-generated, CslSplTokenProgram v0.0.0
	pub fn process_transfer_certification(
		program_id: &Pubkey,
		accounts: &[AccountInfo],
	) -> ProgramResult {
		let account_info_iter = &mut accounts.iter();
		...
		// Calling STUB
		transfer_certification::transfer_certification(
			program_id,
			&vec![
				funding_info,
				assoc_token_account_info,
...
/// 7. `[]` csl_spl_token_v_0_0_0: [AccountInfo] Auto-generated, CslSplTokenProgram v0.0.0
	pub fn process_burn_certification(
		program_id: &Pubkey,
		accounts: &[AccountInfo],
	) -> ProgramResult {
		let account_info_iter = &mut accounts.iter();
		...
		// Calling STUB
		burn_certification::burn_certification(
			program_id,
			&vec![
				account_info,
				mint_info,
