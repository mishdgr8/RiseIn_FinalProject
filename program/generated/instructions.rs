...
pub enum RiseinCertificationInstruction {
/// Accounts:
...
/// - course_description: [String] 
	MintCertification(MintCertificationArgs),
...
pub struct MintCertificationArgs {
...
impl RiseinCertificationInstruction {
    pub fn unpack(input: &[u8]) -> Result<Self, ProgramError> {
        let (&variant, rest) = input.split_first().ok_or(RiseinCertificationError::InvalidInstruction)?;

        Ok(match variant {
			0 => Self::MintCertification(MintCertificationArgs::try_from_slice(rest).unwrap()),
			1 => Self::ClaimCertification,
			2 => Self::TransferCertification,
			3 => Self::BurnCertification,
			_ => return Err(RiseinCertificationError::InvalidInstruction.into())
        })
    }
}