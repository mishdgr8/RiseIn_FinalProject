...
mod mint_certification {
	...
	fn test_transaction_is_ok() {
		...
		let (test_validator, fee_payer_info) = TestValidatorGenesis::default()
			.add_program("risein_certification", program_id)
			.start();
		let rpc_client = test_validator.get_rpc_client();

		let project_name: String = Default::default();
		let my_name: String = Default::default();
		let image_link: String = Default::default();
		...
		let data = RiseinCertificationInstruction::MintCertification(crate::generated::instructions::MintCertificationArgs{
			project_name,
			my_name,
			image_link,
			course_description,
